(function(){
    ...
    var emoji = function(){
        ...
        //colons_mode,text_mode  将表情符替换为适当的表示，替换为他们的冒号字符串表示
    ...
    emoji.prototype.init_env = function(){
        ...
        //需要一个更好的方法来检测Android端对emoji的支持情况
    ...
    emoji.prototype.init_colons = function(){
        ...
        self.map.colons = {};
        for(var i in self.data){    //关于data的定义放在本文件的最后
    ...
    emoji.prototype.init_emoticons = function(){
        ...
        var a = [];
        self.map.emoticons = {};
        for(var i in self.emoticons_data){//emoticons_data定义在最后
            //将&,<,>符号转成html转义字符
    ...
    emoji.prototype.replacement = function(idx,actual,wrapper,variation){
        ...
        //处理图片模式的
        /*
        这里的变量选择有些复杂——如果图片设置和特定的表情支持变量，那么使用变量图片。否则，返回一个已经在'extra'中计算好的变量作为单独的图片
        */
        //首先我们设置一些将要被用到的参数，如果不能使用变量
        ...
        /*
        现在将要看到我们能否使用变量，如果可以的话，我们就要覆盖上面的参数，取消'extra'，所以输出单个象形
        我们需要检查：
        * 申请一个变量
        * 有这样一个变量存在在emoji.variations_data
        * 不能为自己的象形使用自定义的图片
        * 在当前的图片设置中，有图片与变量相对应的被定义
        */
        ...
        return '<img src="' + img + '" class="emoji" style="width:1.2rem" ' + title + '/>';
    };

    //参数是在情感符中可能包含冒号字符串的表述，如(ie. `:smile:`)，返回值是一个包含所有冒号字符串表情符被替代为合适的表述
    ...
    //参数A字符串可能是一个ASCII情感符，如(ie. `:)`)，返回一个在字符串中含有情感符含义的新的字符来替代当前环境支持的表述
    ...
    //表情对应的编码表
    //[[c/c++和Java的src],"Python src","...",["别名"],后面的数字表示在精灵图中的位置,"字符表示"]   这个纯属自己的摸索，不一定准确
    emoji.prototype.data = {
        ...
        "1f44a_1f3fb":[["\uD83D\uDC4A"],"\uE00D","\uDBBA\uDF96",["facepunch","punch"],14,27,15,0],
        "1f44b":[["\uD83D\uDC4B"],"\uE41E","\uDBBA\uDF9D",["wave"],14,33,15,0],
        "1f44c_1f3fb":[["\uD83D\uDC4C"],"\uE420","\uDBBA\uDF9F",["ok_hand"],14,39,15,0],
        "1f44d_1f3fb":[["\uD83D\uDC4D"],"\uE00E","\uDBBA\uDF97",["+1","thumbsup"],15,4,15,0],
    ...
    //大表情的数据
    emoji.prototype.emoticons_big_icon = {
        "87+/sQ==": "laugh",
        "87+/sg==": "like",
        "87+/sw==": "praise",
        "87+/tA==": "daze",
    ...
    /*
    参数是一个可能包含同意unicode编码的表情符，如(ie. 😄)，返回的是一个适合当前环境合适的表示的字符串来代替所有unicode表情符
     */
