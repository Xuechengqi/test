 ...
 * demo：
 * 
 *    	$des = new CryptDes("w1s5de68","548e933ert55");//（秘钥向量，混淆向量）
...
class CryptDes {
	...
	public function __construct($key=null, $iv=null) {
        $this->key = is_null($key)?"w1s5de68":$key;
		$this->iv = is_null($iv)?"548e933ert55":$iv;
    }
	...
	public function decrypt($encrypted){
		//$encrypted = pack("H*",hex2bin($encrypted)); //如需转换二进制可改成  bin2hex 转换
